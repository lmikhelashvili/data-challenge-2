...
new_sun <- park_movement_Sun %>% 
  filter(id == "378256" | id == "297330" | id == "1323905" | id == "1906865" | id == "1784014" ) %>% 
  filter(Timestamp >= "2014-6-08 11:30:00" & Timestamp <= "2014-6-08 12:30:00")
# id=="2057828" | id == "1159870" | id == "1624317" | id == "1713869" | id == "1729147"

```

```{r}
ggplot(data = new_sun, aes(x = X, y = Y, color = factor(id)))+
  geom_point()+
  geom_text(label = Timestamp)
```


378256 and 1784014 talking most during the 11 am timeblock
```{r}
hhh <- data_Sun%>%
  filter((from == "378256" |from == "297330" | from == "1323905" | from == "1906865" | from == "1784014" ) & (to == "297330" | to == "1323905" | to == "1906865" | to == "1784014" ))
```
